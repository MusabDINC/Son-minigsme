...
export default class Resources extends EventEmitter
{
    constructor()
    {
        ...
        // Kaynakları gruplandır (lazy loading için)
        this.resourceGroups = {
            core: [],        // Çekirdek kaynaklar (her zaman yüklenir)
            footballGame: [] // Football mini oyunu kaynakları (ihtiyaç duyulduğunda yüklenir)
        }
        
        // Kaynakları gruplandır
        this.organizeResources()
        
        // Ana kaynakları yükle
        this.loadCoreResources()
        
        // Futbol oyunu kaynaklarının yükleme durumu
        this.footballResourcesLoaded = false
    }
    ...
    organizeResources() {
        ...
        this.resourceGroups.core.push(
         ...
         this.resourceGroups.core.push(
            { name: 'rocketModel', source: './models/rocket/base.gltf' }
        )
            this.resourceGroups.core.push(
            // Intro
            ...
            // Kale modeli - futbol sahası için
            ...
            // Bowling pin modelleri kaldırıldı
    ...
    /**
     * Çekirdek kaynakları yükle
     */
    loadCoreResources() {
        // Çekirdek kaynakları yükle
        this.loader.load(this.resourceGroups.core)
        
        // Her bir kaynak yüklendiğinde olayı tetikle
        this.loader.on('itemLoaded', (_resource, _index) => {
            this.items[_resource.name] = _resource.data
            
            this.trigger('progress', [_index / this.resourceGroups.core.length])
        })
        
        // Tüm kaynaklar yüklendiğinde olayı tetikle
        this.loader.on('end', () => {
    ...
    /**
     * Futbol oyunu kaynaklarını yükle
     */
    loadFootballResources() {
        if(this.footballResourcesLoaded) return
        
        // Futbol kaynaklarını yükle
        this.loader.load(this.resourceGroups.footballGame)
        
        // Kaynaklar yüklendiğinde olayı tetikle
        this.loader.on('end', () => {
            this.footballResourcesLoaded = true
            this.trigger('footballResourcesReady')
        })
    }
}
